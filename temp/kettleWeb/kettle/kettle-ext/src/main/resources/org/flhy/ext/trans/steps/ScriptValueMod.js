ScriptValuesModDialog = Ext.extend(KettleDialog, {
		...
		this.fitItems = {
			...
			items: [{
				...
				columns: [new Ext.grid.RowNumberer(), {
					...
					header: '改名为', dataIndex: 'rename', width: 100, editor: new Ext.form.TextField()
				},{
					...
					header: '长度', dataIndex: 'length', width: 100, editor: new Ext.form.TextField()
				},{
	...
	initBottomBar: function(bar) {
		bar.insert(2, {
			text: '测试脚本', scope: this, handler: function() {
				this.onSure(false);
    ...
    createMenu : function(){
        ...
        if(!this.menu){
            this.menu = new Ext.menu.Menu({
                items: [{
                	text: 'Add New', iconCls: 'addNew', handler: function() {
                		var i = 0, flag = true;
                		while(flag) {
                			flag = false;
                			tabPanel.items.each(function(item) {
                	...
                	text: 'Add Copy', iconCls: 'addCopy'
                }, '-', {
                	text: 'Set Transform Script', iconCls: 'activeScript', handler: function() {
                		var p = tabPanel.getActiveTab();
                		if(p) p.setIconClass('activeScript');
                	}
                }, {
                	text: 'Set Start Script', iconCls: 'activeStartScript', handler: function() {
                		var p = tabPanel.getActiveTab();
                		if(p) p.setIconClass('activeStartScript');
                	}
                }, {
                	text: 'Set End Script', iconCls: 'activeEndScript', handler: function() {
                		var p = tabPanel.getActiveTab();
                		if(p) p.setIconClass('activeEndScript');
                	}
                }, '-', {
                	text: 'Remove Script Type', iconCls: 'scriptType'
                }]
            });
        }
        return this.menu;
