K.AGS.DynamicMapLayer = K.ImageOverlay.extend({
  includes: K.AGS.Mixins.identifiableLayer,
  ...
  initialize: function (url, options) {
    ...
    this._url = url;
    this.serviceUrl = url;
    this._layerParams = K.Util.extend({}, this.defaultParams);
  ...
  onAdd: function (map) {
    ...
    if (map.options.zoomAnimation && K.Browser.any3d) {
  ...
  _animateZoom: function (e) {
    ...
    image.style[K.DomUtil.TRANSFORM] = K.DomUtil.getTranslateString(origin) + ' scale(' + scale + ') ';
  },
  ...
  _initImage: function () {
    this._image = K.DomUtil.create('img', 'leaflet-image-layer');

    if (this._map.options.zoomAnimation && K.Browser.any3d) {
      K.DomUtil.addClass(this._image, 'leaflet-zoom-animated');
    } else {
      K.DomUtil.addClass(this._image, 'leaflet-zoom-hide');
    }

    this._updateOpacity();

    K.Util.extend(this._image, {
      galleryimg: 'no',
      onselectstart: K.Util.falseFn,
      onmousemove: K.Util.falseFn,
      onload: K.Util.bind(this._onImageLoad, this),
  ...
  _getImageUrl: function () {
    ...
    var url = this._url + 'export' + K.Util.getParamString(this._layerParams);
  ...
  _update: function (e) {
    ...
    this._newImage = K.DomUtil.create('img', 'leaflet-image-layer');

    if (this._map.options.zoomAnimation && K.Browser.any3d) {
      K.DomUtil.addClass(this._newImage, 'leaflet-zoom-animated');
    } else {
      K.DomUtil.addClass(this._newImage, 'leaflet-zoom-hide');
    }

    this._updateOpacity();

    K.Util.extend(this._newImage, {
      galleryimg: 'no',
      onselectstart: K.Util.falseFn,
      onmousemove: K.Util.falseFn,
      onload: K.Util.bind(this._onNewImageLoad, this),
      src: this._getImageUrl()
    });
  },
  ...
  _updateOpacity: function(){
    ...
    if(this._newImage){
      K.DomUtil.setOpacity(this._newImage, this.options.opacity);
    }
  },
  ...
  _onNewImageLoad: function () {
    var bounds = this._map.getBounds(),
        nw = K.latLng(bounds._northEast.lat, bounds._southWest.lng),
        se = K.latLng(bounds._southWest.lat, bounds._northEast.lng);

    var topLeft = this._map.latLngToLayerPoint(nw),
        size = this._map.latLngToLayerPoint(se)._subtract(topLeft);
    K.DomUtil.setPosition(this._newImage, topLeft);
...
K.AGS.dynamicMapLayer = function (url, options) {
  return new K.AGS.DynamicMapLayer(url, options);
};