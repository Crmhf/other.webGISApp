...
function Frame() {
    ...
    this.showSetupPanel = function(){
      ...
      //  $('#save-panel').removeClass("hide");
        ...
        $("#input-viz-name").val(appInfo.resource.name);
        $("#text-viz-desc").val(appInfo.resource.description);
        $("#text-viz-info").val(appInfo.resourceExtend.info);
        //, this.mapView.shrinkMap();
    };
...
$(document).ready(function () {
    ...
    $('#btn-add-ok').click(function(){
        var info = $("#text-viz-info").val();
        addAppInfo(info);
    });
    // 编辑应用-绑定界面中的修改应用的功能
    $('#btn-save-ok').click(function(){
        var info = $("#text-viz-info").val();
        editAppInfo(info);
    });
       ...
       $.ajax({
           ...
           // url:'http://192.168.227.106:8080/share-exchange/webservices/rest/serviceregister/queryviewinfo/1/10?_type=json',
            ...
            success:function(data){
                ...
                var content;
                content='<table class="table table-bordered table-hover"><thead> <tr> <th style="width: 10px;"><input type="checkbox"/></th> <th>名称</th> <th>类型</th> </tr> </thead> <tbody>'
                    for(var i=0,len=result.length; i<len; i++){
                ...
                var a=$("#data-layer-list-dialog").html();
                if(a==""){
                    $("#data-layer-list-dialog").append(content);
                }
            },
            error:function(data){
                alert("error");
