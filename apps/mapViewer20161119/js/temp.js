...
function addAppInfo(info){
    ...
    // 提交的数据一定要先格式化
    var appName = $("#input-viz-name").val();
        // $('#appName').val();
    var appDesc =  $("#text-viz-desc").val();
        // $('#appDesc').val();
    var x = uuid();
   ...
   // var data = JSON.stringify({ id: appExId, resourceid: appid, info: info});
    ...
    var typeid = '5',
        ...
        catalogid = '402881bc586194180158621a37cd0006',
        author = '衡阳管理员用户',
        version = '1.0',
        owner = '2';
...
function editAppInfo(info){
    // 获取服务地址
    var requestURL = commonConfig.app.updateAll;
    /*var data = {
     id:"402881f2560308af0156034fce600002",
     resourceid:id,
     info:jQuery.parseJSON('{x:111}')
    ...
    appInfo.resource.name = $("#input-viz-name").val();
    appInfo.resource.description = $("#text-viz-desc").val();
    appInfo.resourceExtend.info = $("#text-viz-info").val();

    // 提交的数据一定要先格式化 提交的是 commonConfig.app.update
   //  var data = JSON.stringify({ id: appExId, resourceid: appid, info: info});

    var data = JSON.stringify({"resource":appInfo.resource,"resourceExtend":appInfo.resourceExtend,"tagid":appInfo.tagid});
...
// 没有获取到地图的相关信息的时候，就从默认的地图信息中获取到相关的信息
var mapItem = {
    defaults: {
